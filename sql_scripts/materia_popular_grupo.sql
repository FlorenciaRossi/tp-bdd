with materias_count as (select  -- cuenta la cantidad de inscriptos en una materia por cada grupo
	g.grupo_id,
    am.materia_id,
...
group by 1,2),
materias_popular_grupo as ( -- traemos solo la cantidad maxima de incriptos en materias por grupo
select grupo_id,max(cant_inscriptos) as cant_inscriptos from materias_count 
group by grupo_id 
),
cantidad_alumnos_grupos as ( -- calculamos la cantidad de alumnos por grupos para sacar el porcentaje
select 
	grupo_id,
...
select  
	...
	concat((mpg.cant_inscriptos*100.0)/cag.cant_alumnos,'%') as porcentaje_inscriptos -- calculamos porcentaje de de alumnos inscriptos
